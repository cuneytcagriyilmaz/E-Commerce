...
const ShoppingCart = () => {
    ...
    return (
        <div className="container mx-auto p-4">
            {cart.map(item => (
                <div key={item.product.id} className="flex flex-col md:flex-row items-start justify-between p-4 border-b">
                    <div className="flex items-start space-x-4 w-full md:w-1/3 mb-4 md:mb-0">
                        ...
                        />
                        <img src={item.product.images[0].url} alt={item.product.name} className="w-16 h-16 rounded" />
                        <div className="flex flex-col">
                            <h2 className="font-semibold">{item.product.name}</h2>
                            <p className="text-sm text-gray-600">{item.product.description}</p>
                            <div className="flex items-center space-x-4 mt-2">
                                <button onClick={() => handleQuantityChange(item.product.id, -1)} className="p-2 bg-gray-200 rounded"><i className="fas fa-minus"></i></button>
                                <span>{item.count}</span>
                                <button onClick={() => handleQuantityChange(item.product.id, 1)} className="p-2 bg-gray-200 rounded"><i className="fas fa-plus"></i></button>
                                <button onClick={() => handleRemoveProduct(item.product.id)} className="p-2 text-neutral-700 rounded"><i className="fas fa-trash-alt"></i></button>
                            </div>
                        </div>
                    </div>
                    <div className="flex flex-col items-center justify-center w-full md:w-1/3 mb-4 md:mb-0">
                        ...
                        <p className="text-sm text-purple-600">{item.product.shippingCost}</p>
                    </div>
